...
- The contact form uses FormSubmit and sends to `philip@ryujinte.com`. The first test submission triggers a one-time activation email at that address; click the activation link before relying on the form. Visible CAPTCHA is disabled and the hidden honeypot remains enabled for basic spam filtering.
...
- Confirm that `philip@ryujinte.com` can both send and receive before promoting the site.
